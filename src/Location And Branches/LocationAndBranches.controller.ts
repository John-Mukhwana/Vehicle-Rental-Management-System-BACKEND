...
export const listLocationAndBranches = async (c: Context) => {
    try {
...
export const getLocationAndBranches = async (c: Context) => {
...
export const createLocationAndBranches = async (c: Context) => {
    try {
        ...
        if (!createLocationAndBranches) return c.text("Location and Branches not created", 404);
...
export const updateLocationAndBranches = async (c: Context) => {
    const id = parseInt(c.req.param("id"));
    if (isNaN(id)) return c.text("Invalid ID", 400);
    ...
    try {
        ...
        if (searchedLocationAndBranches == undefined) return c.text("Location And Branches not found", 404);
        // get the data and update it
        const res = await updateLocationAndBranchesService(id, LocationAndBranches);
        // return a success message
        if (!res) return c.text("Location And Branches not updated", 404);
...
export const deleteLocationAndBranches = async (c: Context) => {
    ...
    try {
        ...
        if (LocationAndBranches == undefined) return c.text("Location And Branches not found", 404);
        //deleting the user
